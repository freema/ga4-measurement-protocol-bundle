...
class BeginCheckoutEvent extends AbstractEvent
{
    ...
    /**
     * Validate the begin checkout event according to GA4 requirements.
     *
    ...
    public function validate(): bool
    {
        ...
        if (!empty($this->getValue())) {
            if (empty($this->getParameter('currency'))) {
                throw new ValidationException('Field "currency" is required when "value" is set', ErrorCode::VALIDATION_BEGIN_CHECKOUT_CURRENCY_REQUIRED_WITH_VALUE, 'currency');
            }
        }

        // At least one item is required
        if (empty($this->items)) {
            throw new ValidationException('At least one item is required for begin_checkout events', ErrorCode::VALIDATION_BEGIN_CHECKOUT_ITEMS_REQUIRED, 'items');
        }
